...
void chebyshev::Moments1D_Graphene_NNN::Print()
{
...
chebyshev::Moments1D_Graphene_NNN::Moments1D_Graphene_NNN( Graphene_NNN& device, std::string momfilename ):Moments_Graphene_NNN(device)
{
	...
	this->MomentVector( Moments_Graphene_NNN::vector_t(_numMoms, 0.0) );
	double rmu,imu;
	for( int m0 = 0 ; m0 < _numMoms ; m0++)
	{ 
		momfile>>rmu>>imu;
		this->operator()(m0) = Moments_Graphene_NNN::value_t(rmu,imu);
	}
	momfile.close();
};


void chebyshev::Moments1D_Graphene_NNN::MomentNumber(const size_t numMoms )
{ 
	...
	chebyshev::Moments1D_Graphene_NNN new_mom(this->device(), numMoms );
...
void chebyshev::Moments1D_Graphene_NNN::ApplyJacksonKernel( const double broad )
{
  assert( broad > 0);
...
void chebyshev::Moments1D_Graphene_NNN::ApplyLorentzKernel( const double broad, const double lambda )
{
  assert( broad > 0);
  const double eta = 2.0*broad/1000/this->BandWidth();
...
void chebyshev::Moments1D_Graphene_NNN::saveIn(std::string filename)
{
