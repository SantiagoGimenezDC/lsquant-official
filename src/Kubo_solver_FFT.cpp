...
void Kubo_solver_FFT_kQuant_nonOrth::compute(   qstates::generator& gen ){

  time_station solver_station;


  const size_t DIM = chebVecL_->SystemSize();
  Kubo_solver_FFT_postProcess_dup postProcess(*this);
  section_size_ = DIM / num_sections_ +
                  DIM % num_sections_; //We are assuming */* is bigger than *%*. If not, i'd increase/decrease num_parts until it is.
  ...
  while( r < R_)  { //gen.getQuantumState()    
    ...
    gen.getQuantumState();    
    //SELECT RUNNING TYPE
    polynomial_recursion(gen.State(),gen.State(),  chebVecL_,chebVecR_, r_data);
...
void Kubo_solver_FFT_kQuant_nonOrth::allocate(Vectors_sliced_kQuant_nonOrth& chebVecL, Vectors_sliced_kQuant_nonOrth& chebVecR){
  const size_t DIM = Hamiltonian_dummyMoms_.SystemSize();
...
void Kubo_solver_FFT_kQuant::compute( SparseMatrixType & OP1, SparseMatrixType & OP2,   qstates::generator& gen ){

  time_station solver_station;


  const size_t DIM = chebVecL_->SystemSize();
  Kubo_solver_FFT_postProcess_dup_dup postProcess(*this);
  section_size_ = DIM / num_sections_ +
                  DIM % num_sections_; //We are assuming */* is bigger than *%*. If not, i'd increase/decrease num_parts until it is.
  ...
  while( r < R_)  { //gen.getQuantumState()    
    ...
    gen.getQuantumState();    
    //SELECT RUNNING TYPE
    polynomial_recursion( gen.State(),gen.State(), OP1, OP2, chebVecL_,chebVecR_, r_data);
...
void Kubo_solver_FFT_kQuant::polynomial_recursion(const vector_t& PhiR, const vector_t& PhiL,
						  SparseMatrixType & OP1, SparseMatrixType & OP2,
				Vectors_sliced_kQuant *chebVecL,
				Vectors_sliced_kQuant *chebVecR,
				value_t r_data[]){
...
void Kubo_solver_FFT_kQuant::allocate(Vectors_sliced_kQuant& chebVecL, Vectors_sliced_kQuant& chebVecR){
  const size_t DIM = Hamiltonian_dummyMoms_.SystemSize();
...
void Kubo_solver_FFT_kQuant_nonOrth_ChrisVel::compute( qstates::generator& gen ){

  time_station solver_station;


  const size_t DIM = chebVecL_->SystemSize();
  Kubo_solver_FFT_postProcess_dup_dup_dup postProcess(*this);
  section_size_ = DIM / num_sections_ +
                  DIM % num_sections_; //We are assuming */* is bigger than *%*. If not, i'd increase/decrease num_parts until it is.
...
void Kubo_solver_FFT_kQuant_nonOrth_ChrisVel::polynomial_recursion(const vector_t& PhiR, const vector_t& PhiL,
				Vectors_sliced_kQuant_nonOrth_ChrisVel *chebVecL,
				Vectors_sliced_kQuant_nonOrth_ChrisVel *chebVecR,
				value_t r_data[]){
...
void Kubo_solver_FFT_kQuant_nonOrth_ChrisVel::allocate(Vectors_sliced_kQuant_nonOrth_ChrisVel& chebVecL, Vectors_sliced_kQuant_nonOrth_ChrisVel& chebVecR){
  const size_t DIM = Hamiltonian_dummyMoms_.SystemSize();
