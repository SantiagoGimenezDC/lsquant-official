...
int chebyshev::MomentsTD_Device::Deltaiter(double E)
{
...
*/
	...
	//From now on this-> will be discarded in Chebyshev0() and Chebyshev1()
  	...
  	double g_D_m=chebyshev::Moments_Device::JacksonKernel(currentTm,  NumMoms );
...
int chebyshev::MomentsTD_Device::Evolve( vector_t& Phi)
{
	...
	if( this->Chebyshev2().size()!= dim )
		this->Chebyshev2() = Moments_Device::vector_t(dim,Moments_Device::value_t(0)); 

	
	auto tmp = Moments_Device::vector_t(dim,Moments_Device::value_t(0)); 
	auto tmp2 = Moments_Device::vector_t(dim,Moments_Device::value_t(0)); 
	auto tmp3 = Moments_Device::vector_t(dim,Moments_Device::value_t(0)); 
	...
	int n = 0;
	auto nIp =Moments_Device::value_t(1, 0);
...
int chebyshev::MomentsTD_Device::MSD_Evolve(vector_t& Phi)
{
	const auto dim = this->SystemSize();
	auto PhiT = Moments_Device::vector_t(dim,Moments_Device::value_t(0)); 
...
int chebyshev::MomentsTD_Device::Conv_Evolve( vector_t& Phi)
{
	//Conmuter=-Conmuter;
	const auto dim = this->SystemSize();
	...
	if( this->Chebyshevx0().size()!= dim )
		this->Chebyshevx0() = Moments_Device::vector_t(dim,Moments_Device::value_t(0)); 

	if( this->Chebyshev1().size()!= dim )
		this->Chebyshev1() = Moments_Device::vector_t(dim,Moments_Device::value_t(0)); 

	if( this->Chebyshevx1().size()!= dim )
		this->Chebyshevx1() = Moments_Device::vector_t(dim,Moments_Device::value_t(0)); 
...
chebyshev::MomentsTD_Device::MomentsTD_Device(Device& device,  std::string momfilename):Moments_Device(device)
{
  ...
  this->MomentVector( Moments_Device::vector_t( this->HighestMomentNumber() * this->MaxTimeStep(), 0.0) );
  double rmu, imu;
  for( int m = 0; m < this->HighestMomentNumber() ; m++)
  for( int n = 0; n < this->MaxTimeStep() ; n++)
  { 
	  momfile>>rmu>>imu;
	  this->operator()(m,n) = Moments_Device::value_t(rmu, imu);
  }
  momfile.close();
};

void chebyshev::MomentsTD_Device::saveIn(std::string filename)
{
...
void chebyshev::MomentsTD_Device::Print()
{
...
void chebyshev::MomentsTD_Device::MomentNumber(const size_t numMoms )
{ 
	...
	chebyshev::MomentsTD_Device new_mom(this->device(), numMoms, maxtime);
...
void chebyshev::MomentsTD_Device::ApplyJacksonKernel( const double broad )
{
  assert( broad > 0);
