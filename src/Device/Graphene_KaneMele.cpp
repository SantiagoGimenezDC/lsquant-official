...
Graphene_KaneMele::Graphene_KaneMele(int is_k_space, r_type range, r_type stgr_str, r_type m_str, r_type rashba_str, r_type KM_str, r_type HLD_str, device_vars& parameters):
  Device(parameters), stgr_str_(stgr_str), m_str_(m_str), rashba_str_(rashba_str), KM_str_(KM_str){
    
  this->parameters().DIM_*=4;
  this->parameters().SUBDIM_*=4;
  this->parameters().DIS_DIM_ = this->parameters().DIM_;
  ...
  r_type t            = t_standard_,
    ...
    KM_str_2     = -KM_str_ , //This matches Joaquin's KaneMele
         HLD_str_2    = HLD_str ;
  ...
  */
  ...
  this->parameters().DOS_corr_ = 1.0 / ( Length * Length )  ; // 8.2 here should be the full bandwidth.
...
void Graphene_KaneMele::build_Hk(){
  ...
  */
  ...
  //==========================CHATGPT SCRIPT---------------------//
  std::vector<Eigen::Vector2d> local_lists[omp_get_max_threads()];

#pragma omp parallel
{
  int tid = omp_get_thread_num();
  std::vector<Eigen::Vector2d>& my_list = local_lists[tid];

  size_t local_subdim = 0;
...
for (int t = 0; t < omp_get_max_threads(); ++t) {
  subdim += local_lists[t].size();
  nonZeroList_.insert(nonZeroList_.end(), local_lists[t].begin(), local_lists[t].end());
}
//================================END CHATGPT SCRIPT ================================//
...
#pragma omp parallel for    
  ...
  // this->set_sysSubLength(Length * sqrt(double(this->parameters().SUBDIM_)/double(this->parameters().DIM_)));
  //this->set_sysLength(Length  * sqrt(double(this->parameters().SUBDIM_)/double(this->parameters().DIM_)));
...
Eigen::MatrixXcd Graphene_KaneMele::vk_single(Eigen::Vector2d k) {
    ...
    v_k.block(0,0,4,4) = -(v_k.block(0,0,4,4)  + v_k_adj); //Minus sign?? 
...
void Graphene_KaneMele::J (type* ket, type* p_ket, int dir){
  ...
  Eigen::Matrix2cd sx{{0,1},{1,0}}, sy{{0,type(0,-1.0)}, {type(0,1), 0}}, sz{{1,0}, {0, -1}}, chosen_dir; 
...
#pragma omp parallel for 
 for(int i=0; i<SUBDIM/4; i++){
     ...
     eig_ket.segment(4*i,2)   =  chosen_dir * eig_p_ket.segment(4*i,2);
     eig_ket.segment(4*i+2,2) =  chosen_dir * eig_p_ket.segment(4*i+2,2);
...
void Graphene_KaneMele::spin_project (type* ket, int dir){

  int SUBDIM = this->parameters().SUBDIM_;
  

  
  Eigen::Matrix2cd sx{{0,1},{1,0}}, sy{{0,type(0,-1.0)}, {type(0,1), 0}}, sz{{1,0}, {0, -1}}, id{{1,0}, {0, 1}}, chosen_dir; 
  ...
  //  Eigen::VectorXcd eig_p_ket=Eigen::VectorXcd::Zero( SUBDIM);
...
#pragma omp parallel for 
 for(int i=0; i<SUBDIM/4; i++){
   ...
   eig_ket.segment(4*i,2)   = ( id + chosen_dir ) / 2.0 * eig_ket.segment(4*i,2);
   eig_ket.segment(4*i+2,2) = ( id + chosen_dir ) / 2.0 * eig_ket.segment(4*i+2,2);
...
void Graphene_KaneMele::rearrange_initial_vec(type* r_vec){
  ...
  Eigen::Map<Eigen::VectorXcd> eig_r_vec(r_vec,dis_dim);
  
  Eigen::VectorXcd eig_rearranged__r_vec = Eigen::VectorXcd::Zero(subDim);
...
#pragma omp parallel for  
  for (size_t i = 0; i < nonZeroList_.size(); i++ ) {
    ...
    for(size_t j = 0; j < 4; j++)
      eig_rearranged__r_vec[ i*4 + j ] = r_vec[ ( i2 + j2 * W ) * 4 + j ];

  }

  
  eig_r_vec = eig_rearranged__r_vec;
};
...
void Graphene_KaneMele::print_hamiltonian(){
  ...
  dataP.open("vx_diff.txt");
	...
	to_kSpace_pruned(term_j_2.data(), term_j_2.data());
	...
	to_rSpace_pruned(tmp.data(), tmp.data());
...
void Graphene_KaneMele::projector(type* ket ){
  ...
  for(size_t i = 0; i < size_t( diag.size() ); ++i) 
...
#pragma omp parallel for 
 for(size_t j=0; j<Le; j++){
    for(size_t i=0; i<W; i++){      
      ...
      eig_ket.segment(n1,4) *= 2.0; //weird factor of 2
...
#pragma omp parallel for 
 for(size_t j=0; j<Le; j++){
    for(size_t i=0; i<W; i++){      
      ...
      eig_ket.segment(n1,4) =  b_a  * eig_p_ket.segment(n1,4)  - 0.25 * eig_pp_ket.segment(n1,4); //weird factor of 2
      ...
      eig_ket.segment(n1,4) *= 4.0; //weird factor of 2
