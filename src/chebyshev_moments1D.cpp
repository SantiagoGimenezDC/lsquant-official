...
void chebyshev::Moments1D_kQuant::Print()
{
	std::cout<<"\n\nCHEBYSHEV 1D MOMENTS INFO"<<std::endl;
	std::cout<<"\tSYSTEM:\t\t\t"<<this->SystemLabel()<<std::endl;
	if( this-> SystemSize() > 0 )
...
chebyshev::Moments1D_kQuant::Moments1D_kQuant( std::string momfilename )
{
	//Check if the input_momfile have the right extension 
	std::size_t ext_pos = string( momfilename ).find(".chebmom1D"); 
	if( ext_pos == string::npos )
...
void chebyshev::Moments1D_kQuant::MomentNumber(const size_t numMoms )
{ 
	assert( _numMoms <= this->HighestMomentNumber() );
	...
	this->SetMomentNumber(new_mom.MomentNumber());
        //_numMoms = new_mom._numMoms;
	this->MomentVector( new_mom.MomentVector() );
};


void chebyshev::Moments1D_kQuant::ApplyJacksonKernel( const double broad )
{
  assert( broad > 0);
  const double eta = 2.0*broad/1000/this->BandWidth();
  int maxMom =  ceil(M_PI/eta);
...
void chebyshev::Moments1D_kQuant::saveIn(std::string filename)
{
  typedef std::numeric_limits<double> dbl;
  ofstream outputfile(filename.c_str());
  assert(outputfile.is_open());
...
void chebyshev::Moments1D_kQuant_nonOrth::Print()
{
	std::cout<<"\n\nCHEBYSHEV 1D MOMENTS INFO"<<std::endl;
	std::cout<<"\tSYSTEM:\t\t\t"<<this->SystemLabel()<<std::endl;
	if( this-> SystemSize() > 0 )
...
chebyshev::Moments1D_kQuant_nonOrth::Moments1D_kQuant_nonOrth( std::string momfilename )
{
	//Check if the input_momfile have the right extension 
	std::size_t ext_pos = string( momfilename ).find(".chebmom1D"); 
	if( ext_pos == string::npos )
...
void chebyshev::Moments1D_kQuant_nonOrth::MomentNumber(const size_t numMoms )
{ 
	assert( _numMoms <= this->HighestMomentNumber() );
...
void chebyshev::Moments1D_kQuant_nonOrth::ApplyJacksonKernel( const double broad )
{
  assert( broad > 0);
  const double eta = 2.0*broad/1000/this->BandWidth();
  int maxMom =  ceil(M_PI/eta);
...
void chebyshev::Moments1D_kQuant_nonOrth::saveIn(std::string filename)
{
  typedef std::numeric_limits<double> dbl;
  ofstream outputfile(filename.c_str());
  assert(outputfile.is_open());
