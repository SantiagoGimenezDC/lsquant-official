...
public:
    ...
    // Bond displacement vectors (same spinful layout as nn / nnn)
    // Used by XYH_ket to compute the position commutators [X,H] and [Y,H]
    CMat dx;    // (natoms x maxnn)  x-displacement to 1st neighbours
    CMat dy;    // (natoms x maxnn)  y-displacement to 1st neighbours
    CMat ddx;   // (natoms x maxnnn) x-displacement to 2nd neighbours
    CMat ddy;   // (natoms x maxnnn) y-displacement to 2nd neighbours
  ...
  Graphene_NNN(int size, bool isDummy )
    ...
    // ── H|ket> ───────────────────────────────────────────────────────────────
    void Multiply(double f1, cdouble* ket, double f2, cdouble* p_ket)
    {
  
        #pragma omp parallel for schedule(dynamic)
        for (int i = 0; i < natoms; ++i) {
            cdouble val = e(i) * ket[i];
            for (int j = 0; j < near_n(i);  ++j) val += s(i,j)  * ket[nn(i,j)];
            for (int j = 0; j < nnear_n(i); ++j) val += ss(i,j) * ket[nnn(i,j)];
            p_ket[i] = f1 * (val - b * ket[i]) / a + f2 * p_ket[i];
        }
    }
  ...
  // ── H|ket> ───────────────────────────────────────────────────────────────
    ...
    void mv_2mu(cdouble* r_n, cdouble* r_nm1, cdouble& suma, cdouble& sumb) const
    {

      suma=0.0;
      sumb=0.0;

      double re_suma = 0.0, im_suma = 0.0, re_sumb = 0.0;
      
        #pragma omp parallel for schedule(dynamic) \
	  reduction(+: re_suma, im_suma, re_sumb)
        for (int i = 0; i < natoms; ++i) {
            // H_bar * r_n[i]  (with adimensionalisation)
            cdouble cpsi = e(i) * r_n[i];
            for (int j = 0; j < near_n(i);  ++j) cpsi += s(i,j)  * r_n[nn(i,j)];
            for (int j = 0; j < nnear_n(i); ++j) cpsi += ss(i,j) * r_n[nnn(i,j)];

            cpsi = 2.0 * (cpsi - b * r_n[i]) / a - r_nm1[i];
	    r_nm1[i] =  cpsi;

	    cdouble prod = std::conj(cpsi) * r_n[i]; // <psi|H_bar|psi> 
	    re_suma += std::real( prod); 
	    im_suma += std::imag( prod ); 
	    re_sumb += std::real( std::conj(cpsi) * cpsi ); // <r_n|r_n>

        }

	suma = cdouble(re_suma, im_suma);
    ...
    // ── [X, H_bar]|ket>  and  [Y, H_bar]|ket>  (position commutators) ────────
    //
    // Computes in a single pass:
    //   xresult[i] = -1/a * ( sum_j  dx(i,j)  * s(i,j)  * ket[nn(i,j)]
    //                        + sum_jj ddx(i,jj) * ss(i,jj) * ket[nnn(i,jj)] )
    //   yresult[i] = same with dy / ddy
    //
    // The 1/a factor comes from H_bar = (H-b)/a: [X, H_bar] = [X,H]/a.
    // The on-site term e(i)*ket[i] commutes with X and contributes nothing.
    void XYH_ket(cdouble* xresult, cdouble* yresult, const cdouble* ket) const
    {
        #pragma omp parallel for schedule(dynamic)
        for (int i = 0; i < natoms; ++i) {
            cdouble xval{0.0, 0.0}, yval{0.0, 0.0};

            for (int j = 0; j < near_n(i); ++j) {
                cdouble w = s(i,j) * ket[nn(i,j)];
                xval -= dx(i,j)  * w;
                yval -= dy(i,j)  * w;
            }
            for (int j = 0; j < nnear_n(i); ++j) {
                cdouble w = ss(i,j) * ket[nnn(i,j)];
                xval -= ddx(i,j) * w;
                yval -= ddy(i,j) * w;
            }
  ...
  void vy_ket( cdouble* vy_ket, const cdouble* ket) const
    {
        #pragma omp parallel for schedule(dynamic)
        for (int i = 0; i < natoms; ++i) {
            cdouble  yval{0.0, 0.0};
    ...
    void vx_ket(cdouble* vx_ket, const cdouble* ket) const
    {
        #pragma omp parallel for schedule(dynamic)
        for (int i = 0; i < natoms; ++i) {
            cdouble xval{0.0, 0.0};
...
private:
    ...
    void build(const UnitCell& uc, const Params& p)
    {
        ...
        // ── 4b. Spin-double the displacement matrices ─────────────────────────
        // Layout mirrors nn_f: diagonal columns carry orbital displacements,
        // off-diagonal (Rashba) columns carry the same displacements.
        Eigen::MatrixXd dx_f(N, mn),   dy_f(N, mn);
        Eigen::MatrixXd ddx_f(N, mn3), ddy_f(N, mn3);
        dx_f.setZero(); dy_f.setZero(); ddx_f.setZero(); ddy_f.setZero();

        for (int i = 0; i < No; ++i) {
            int nup = near_o(i), nnup = nnear_o(i);
            for (int j = 0; j < nup; ++j) {
                // diagonal and off-diagonal (Rashba) share the same displacement
                dx_f(i,    j) = dx_f(i,    nup+j) = dx_o(i,j);
                dx_f(No+i, j) = dx_f(No+i, nup+j) = dx_o(i,j);
                dy_f(i,    j) = dy_f(i,    nup+j) = dy_o(i,j);
                dy_f(No+i, j) = dy_f(No+i, nup+j) = dy_o(i,j);
            }
            for (int j = 0; j < nnup; ++j) {
                ddx_f(i,    j) = ddx_f(i,    nnup+j) = ddx_o(i,j);
                ddx_f(No+i, j) = ddx_f(No+i, nnup+j) = ddx_o(i,j);
                ddy_f(i,    j) = ddy_f(i,    nnup+j) = ddy_o(i,j);
                ddy_f(No+i, j) = ddy_f(No+i, nnup+j) = ddy_o(i,j);
            }
        }
