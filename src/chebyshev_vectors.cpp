...
int chebyshev::Vectors_sliced_kQuant_nonOrth::IterateAllSliced(int s )
{
  ...
  size_t segment_size = ( s == num_sections_-1 ? last_section_size_ : section_size_ ),
	...
	for(int m=1; m < this->NumberOfVectors(); m++ )
	{
	  ...
	  linalg::extract_segment( Chebyshev1(),  segment_start, Vector(m));
	  this->Ham()->Multiply(2.0,Chebyshev1(),-1.0,Chebyshev0());
	  Chebyshev0().swap(Chebyshev1());
...
void chebyshev::Vectors_sliced_kQuant::SetInitVectors_2( SparseMatrixType &OP , const Moments::vector_t& T0 )
{
...
void chebyshev::Vectors_sliced_kQuant::SetInitVectors_2( const Moments::vector_t& T0 )
{
...
int chebyshev::Vectors_sliced_kQuant::MultiplySliced(SparseMatrixType &OP,  int s)
{
  size_t segment_size = ( s == num_sections_-1 ? last_section_size_ : section_size_ ),
    segment_start = s * section_size_,
    DIM = this->SystemSize();
...
int chebyshev::Vectors_sliced_kQuant_nonOrth_ChrisVel::IterateAllSliced(int s )
{
...
int chebyshev::Vectors_sliced_kQuant_nonOrth_ChrisVel::MultiplySliced(  int s)
{
  size_t segment_size = ( s == num_sections_-1 ? last_section_size_ : section_size_ ),
    segment_start = s * section_size_,
    DIM = this->SystemSize();
