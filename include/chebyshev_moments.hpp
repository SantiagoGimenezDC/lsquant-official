...
class Moments_kQuant_nonOrth
{
	...
	inline
	SparseMatrixType_kQuant_nonOrth& Hamiltonian()
	{ 
		return *_pNHAM; 
	};
	...
	//SETTERS
	inline
	void SetHamiltonian( SparseMatrixType_kQuant_nonOrth& NHAM )
	{ 
		if ( this->SystemSize() == 0 ) //Use the rank of the hamiltonian as system size
			this->SystemSize( NHAM.rank() );	
		assert( NHAM.rank() == this->SystemSize()  );
	...
	inline
	void SetAndRescaleHamiltonian(SparseMatrixType_kQuant_nonOrth& NHAM)
	{ 
		this->SetHamiltonian(NHAM );
		this->Rescale2ChebyshevDomain();
	};
	...
	virtual void SetInitVectors( SparseMatrixType_kQuant_nonOrth & ,const vector_t&  );
	...
	private:
	SparseMatrixType_kQuant_nonOrth* _pNHAM;
...
class Vectors_sliced_kQuant : public Moments_kQuant
{
	public: 
	typedef VectorList< Moments::value_t > vectorList_t;
  ...
  //virtual void SetInitVectors( SparseMatrixType & ,const vector_t&  );
	...
	~Vectors_sliced_kQuant(){};  
        ...
        Vectors_sliced_kQuant( chebyshev::Vectors_sliced_kQuant& vec){ *this = vec; };
  
        Vectors_sliced_kQuant( const int numVecs, const size_t num_sections): numVecs_(numVecs), num_sections_(num_sections), section_size_(0), last_section_size_(0), Chebmu_(0,0) {};
	...
	Moments::value_t& operator()(const size_t m0) { return this->Chebmu_(m0,0); };
        ...
        void operator=( Vectors_sliced_kQuant& vec)
	{
		this->SetHamiltonian( vec.Hamiltonian() ) ; 
		this->SystemLabel( vec.SystemLabel());
		this->BandWidth( vec.BandWidth() );
...
class Moments1D_kQuant_nonOrth: public Moments_kQuant_nonOrth
{
	public: 

	Moments1D_kQuant_nonOrth():_numMoms(0){};

	Moments1D_kQuant_nonOrth(const size_t m0):_numMoms(m0){ this->MomentVector( Moments::vector_t(_numMoms, 0.0) ); };

	Moments1D_kQuant_nonOrth( std::string momfilename );
