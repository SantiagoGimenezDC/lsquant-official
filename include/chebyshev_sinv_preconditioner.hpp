#ifndef CHEBYSHEV_PRECONDITIONER_HPP
#define CHEBYSHEV_PRECONDITIONER_HPP

// chebyshev_sinv_preconditioner.hpp
//
// Approximates S^{-1} * vec via a Chebyshev polynomial expansion of the
// scalar function f(x) = 1/x on S's spectrum, evaluated block-by-block for
// the nk x n_orb block-diagonal DFT supercell overlap matrix S:
//
//   1/x  ~=  c_0/2 + sum_{k=1}^{P} c_k T_k(t),      t = (x - center)/halfwidth
//
// where [a,b] (a>0 since S is Hermitian positive definite) brackets S_k's
// spectrum, center=(a+b)/2, halfwidth=(b-a)/2, and c_k are the Chebyshev
// coefficients of 1/x on [a,b] (computed once, analytically/via quadrature,
// NOT via stochastic KPM moments -- this is a direct function expansion).
//
// Applying this to a vector needs only matrix-vector products with the
// rescaled operator  S^_k = (S_k - center*I)/halfwidth , via the standard
// three-term Chebyshev recursion
//
//   v_0 = vec
//   v_1 = S^_k v_0
//   v_{m+1} = 2 S^_k v_m - v_{m-1}
//
//   S_k^{-1} vec  ~=  c_0/2 * v_0 + sum_{m=1}^{P} c_m * v_m
//
// -- no factorization, no linear solve, just P sparse MVMs per block. This
// is the tradeoff: cheap per-application cost, but P must be large enough
// for the target accuracy, and P grows with S_k's condition number since
// 1/x has a singularity approached as the lower band edge a -> 0.
//
// Interface matches IschoPreconditioner / PerfectPreconditioner:
//   ChebyshevSInvPreconditioner<IndexType> C(nk, n_orb);
//   C.compute(S);
//   Vector x = C.solve(b);     // ~= S^{-1} b
...
class ChebyshevSInvPreconditioner
{
public:
    ...
    //                (-1,1) (avoids Chebyshev endpoint/Gibbs issues and
    //                covers any estimation error in a/b themselves)
    ChebyshevSInvPreconditioner(IndexType nk, IndexType n_orb,
    ...
    {
        if (nk <= 0 || n_orb <= 0)
            throw std::invalid_argument("ChebyshevSInvPreconditioner: nk and n_orb must be positive");
        if (maxPolyOrder < 1)
            throw std::invalid_argument("ChebyshevSInvPreconditioner: maxPolyOrder must be >= 1");
    ...
    // Chebyshev coefficients of 1/x on [a,b], and truncate adaptively.
    // ---------------------------------------------------------------
    void compute(const SparseMatS& S)
    {
        if (S.rows() != S.cols())
            throw std::invalid_argument("ChebyshevSInvPreconditioner::compute: S must be square");
        if (S.rows() != m_n)
            throw std::invalid_argument(
                "ChebyshevSInvPreconditioner::compute: S size (" + std::to_string(S.rows()) +
        ...
        for (IndexType k = 0; k < m_nk; ++k)
        {
            ...
            // (fine for typical n_orb; swap for a few Lanczos iterations
            // if n_orb ever gets too large for a dense eigensolve.)
	    ...
	    //std::cout<<"Lambda Min:  "<<lambdaMin<<"   Lambda max: "<<lambdaMax<<std::endl;
	    
            if (lambdaMin <= 0.0)
            {
                // 1/x has a singularity here -- the expansion is not
                // meaningful. S must be Hermitian positive definite.
                #ifdef _OPENMP
                #pragma omp critical
                #endif
                { m_failedBlocks.push_back(k); }
	    ...
	    /*	    
	    const double Delta = std::sqrt(center*center - halfwidth*halfwidth);

	    const double pref = 2.0 * (center - Delta) / (halfwidth * Delta);
	    const double q    = (center - Delta) / halfwidth;
	    // equivalent:
	    // const double q = halfwidth / (center + Delta);

	    std::vector<RealScalar> coeffsFull(m_maxPolyOrder + 1, 0.0);
	    coeffsFull.resize( m_maxPolyOrder + 1);

	    coeffsFull[0] = pref;

	    double qm = -q;
	    for (int m = 1; m <=  m_maxPolyOrder; ++m) {
	      coeffsFull[m] = pref * qm;
	      qm *= -q;
	    }
	    */

	    
            // --- Chebyshev coefficients of f(x)=1/x on [a,b] ---------
            ...
            // --- adaptive truncation: smallest order p such that all
            // trailing coefficients (m > p) are negligible relative to c_0
            ...
            for (int m = m_maxPolyOrder; m >= 1; --m)
            {
                ...
                effOrder = 0; // all the way down to just the constant term
            }
            effOrder = std::max(effOrder, 1); // keep at least T_0, T_1
        ...
        {
            std::string msg = "ChebyshevSInvPreconditioner::compute: block(s) not usable "
    ...
    void compute(const SparseMatS* S)
    {
        if (!S) throw std::invalid_argument("ChebyshevSInvPreconditioner::compute: null S pointer");
        compute(*S);
    }
    ...
    // ---------------------------------------------------------------
    // solve(vec): apply the Chebyshev expansion of S^{-1} block-by-block.
    // ---------------------------------------------------------------
    Vector solve(const Vector& vec) const
    {
        if (!m_computed)
            throw std::runtime_error("ChebyshevSInvPreconditioner::solve: compute() has not been called");
        if (vec.size() != m_n)
            throw std::invalid_argument("ChebyshevSInvPreconditioner::solve: size mismatch");
